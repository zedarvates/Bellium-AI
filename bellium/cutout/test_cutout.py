...
def run_tests():
    ...
    print(
        f"  coverage: {m.coverage_ratio:.3f}, confidence: {m.confidence}, recommendation: {m.recommendation}"
    )
    ...
    draw_a.rectangle(
        [0, 0, 99, 99], fill=(125, 125, 125)
    )  # Touching edges with almost zero contrast
    res_a = extract_foreground(ambig, tolerance=10)
    print(
        f"Test 3 - Low contrast / edge bleed: recommendation={res_a.metrics.recommendation}, conf={res_a.metrics.confidence}"
    )
